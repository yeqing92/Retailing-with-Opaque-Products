# Finalized version of Figure 5 and 6
# July 2023 by Yeqing Zhou

"""
The code contains 3 parts:
1) the supporting functions,
2) calculate profit for each demand setting,
3) plot
"""
...
"""
Part 2: compute for 4 different demand distributions
"""
c = 50
mean = 100
stdev = 20
...
result_tr[demand]= dict.fromkeys(Klist,[])
result_op[demand]= dict.fromkeys(Klist,[])
pList = np.arange(100, 126.5, 0.1)
dList = np.arange(3, 11, 0.1)
for K in Klist:
...
result_tr[demand]= dict.fromkeys(Klist,[])
result_op[demand]= dict.fromkeys(Klist,[])
pList = np.arange(99, 123, 0.1)
dList = np.arange(0.1, 6, 0.1)
for K in Klist:
    resultList = result(demand, demand_input, c, arr_rate, h, K, pList, SList, dList)
    result_tr[demand][K]=[ i for i in resultList[0]]
    result_op[demand][K]=[ i for i in resultList[1]]
...
result_tr[demand]= dict.fromkeys(Klist,[])
result_op[demand]= dict.fromkeys(Klist,[])    
pList = np.arange(99.5, 120, 0.1)
dList = np.arange(0.1, 20, 0.1)
for K in Klist:
    resultList = result(demand, demand_input, c, arr_rate, h, K, pList, SList, dList)
    result_tr[demand][K]=[ i for i in resultList[0]]
    result_op[demand][K]=[ i for i in resultList[1]]
...
fig.savefig('fig_profit_fixedS200_20.pdf',bbox_inches='tight') 
